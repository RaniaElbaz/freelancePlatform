...
export default function UpdateArray({ show, onHide, editKey, userDetails }) {
  ...
  const handleSubmit = () => {
    if (editKey === "skills") {
      if (detailsArray.length > 0) {
        ...
        updateFreelancerArrays();
      } else {
        setErrorMessage({ ...errorMessage, skills: "please add skills first" });
        dataToBeSent.skills = [];
      }
    }
